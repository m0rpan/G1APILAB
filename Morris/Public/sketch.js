...
async function getDataP_edible(){
  const res = await fetch('/api/PlantsEdible');
  const myDataEdible = await res.json();
  console.log(myDataEdible);

  showList_WithImg(myDataEdible)

}

function showList_WithImg(data){
  const dataDiv2= document.getElementById("showDataEdible");
  const list = document.createElement("ul");
